...
import type { AddStoryDialogProps } from "./types";
import { addStoryFormSchema, type AddStoryFormValues } from "./types";
...
export function AddStoryDialog({ onAddStory }: AddStoryDialogProps) {
  ...
  async function onSubmit(values: AddStoryFormValues) {
    try {
      setIsLoading(true);
      await onAddStory({
        ...values,
  ...
  async function generateDescriptionWithAI(
    ...
    if (value === "") {
      toast.error("Please enter the story content first");
    ...
    const description = await improveText({
      text: value,
      system: `You are an expert writer. You will be given a story in English or Arabic. You will need to generate a short description for the story. The description should be short around 20 words. If the text is in English, the description should be in English. If the text is in Arabic, the description should be in Arabic.
        

      Guidlines:
      - Use postive words
      - Use hopeful words and focus on the positive side of the story
      - The description should be about the child and the parent
      - Only describe the family but not the story. Don't say "Beautiful story" for example

      A good description should like this for example:
      "The Story of Rasha and Mustafa and their journey to find a new home"
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new story to be displayed on the website.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <TipTapEditor
                        content={field.value || ""}
                        onChange={field.onChange}
                        placeholder="Enter the story content in Arabic"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          Show this story in featured sections
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
