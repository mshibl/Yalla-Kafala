...
import type { EditStoryDialogProps } from "./types";
import { editStoryFormSchema, type EditStoryFormValues } from "./types";
import { Switch } from "@/components/ui/switch";
import { TipTapEditor } from "@/components/TiptapEditor";
import Image from "next/image";

export function EditStoryDialog({ story, onUpdate }: EditStoryDialogProps) {
  const [open, setOpen] = useState(false);
  const [preview, setPreview] = useState<string | null>(story.imageUrl);
  ...
  async function onSubmit(values: EditStoryFormValues) {
    try {
      setIsLoading(true);
      await onUpdate({
        ...values,
